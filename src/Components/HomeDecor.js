...
const HomeDecor = () => {
  ...
  const [sethomeDecor, setSelectedHomeDecor] = useState()
  const handleSubmit = (event) => {
    axios.get("http://localhost:8080/shop/furniture/" + category)
      .then((res) => {
  ...
  return (
    <div>
      ...
      <div className='container'>
        <br></br>
        <h1 className='text-center' style={{ fontSize: '28px', color: '#84cdee', fontWeight: 'bold' }}><font style={{ color: "black" }}>H</font>OME DECOR</h1>
        <br></br>
        <div className='row gy-5 p-3' style={{width : '1500px', rowGap : '30px'}}>
          {Array.isArray(sethomeDecor) && sethomeDecor.map((formData) => (
            <div className='col-3 m-2 p-3' style={{backgroundColor : '#edf7fc',boxShadow : "10px 10px 10px black", borderRadius : "20px", height : "450px"}}>
              <div className='row'>
                ...
                <img src={`http://localhost:8080/uploads/${formData.image}`} style={{borderTopLeftRadius: '10px', borderTopRightRadius : '10px',height : '200px', width : "330px"}} alt="Image not Found!" />
             </div>
              ...
              <div className='row p-3'>
               ...
               <div className='col'>    <button onClick={() => WishSend({ formData })} style={{borderRadius : '10px', padding : '5px', width:'130px'}}>Add to WishList</button></div>
             </div>
              </div>
